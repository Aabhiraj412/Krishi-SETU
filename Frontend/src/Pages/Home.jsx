...
const Home = () => {
  ...
  const { t } = useLanguage(); // <-- useLanguage hook

  const features = [
    { icon: FaLanguage, textKey: "multiLanguageSupport" },
    { icon: FaMicrophone, textKey: "voiceQueries" },
    { icon: FaBrain, textKey: "aiSolutions" },
    { icon: IoLocationSharp, textKey: "locationAware" },
  ];
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <FaLeaf
          className={`absolute top-20 left-10 transform rotate-45 ${
            isDarkMode ? "text-green-200 opacity-20" : "text-green-400 opacity-10"
          }`}
          size={80}
        />
        <FaRobot
          className={`absolute bottom-20 right-10 ${
            isDarkMode ? "text-blue-200 opacity-20" : "text-blue-400 opacity-10"
          }`}
          size={60}
        />
        <FaCloud
          className={`absolute top-40 right-20 ${
            isDarkMode ? "text-gray-200 opacity-20" : "text-gray-400 opacity-10"
          }`}
          size={70}
        />
        <div
          className={`absolute -top-32 -left-32 w-64 h-64 rounded-full ${
            isDarkMode ? "bg-green-500 opacity-5" : "bg-green-400 opacity-5"
          }`}
        ></div>
        <div
      ...
      <div className="container mt-12 mx-auto px-4 py-12 relative z-10">
            ...
            {features.map((feature, index) => (
              ...
              >
                ...
                <p className="text-sm font-medium text-gray-600 dark:text-gray-400">
                  {t(feature.textKey)} {/* dynamic feature text */}
                </p>
              </div>
            ))}
          </div>
        ...
        <div className="text-center mb-12">
          <p className="text-lg text-gray-600 dark:text-gray-400 max-w-3xl mx-auto leading-relaxed">
            {t("missionStatement")} {/* dynamic mission statement */}
          </p>
        </div>
            ...
            <div className="text-center">
              ...
              <div className="space-y-4">
                <div className="p-3 rounded-lg bg-opacity-10 backdrop-blur-sm">
                  ...
                  <p className="text-sm">{t("askQuestionsLocalLang")}</p>
                </div>
                <div className="p-3 rounded-lg bg-opacity-10 backdrop-blur-sm">
                  <FaDatabase className="text-2xl text-blue-500 mx-auto mb-2" />
                  <p className="text-sm">{t("accessKnowledge")}</p>
                </div>
                <div className="p-3 rounded-lg bg-opacity-10 backdrop-blur-sm">
                  <IoLocationSharp className="text-2xl text-blue-500 mx-auto mb-2" />
                  <p className="text-sm">{t("locationRecommendations")}</p>
                </div>
              </div>
            </div>
          </div>
